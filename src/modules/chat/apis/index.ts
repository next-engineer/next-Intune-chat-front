 ...
 */
import { chatAxiosInstance } from '../../../commons/apis/axiosInstance.api';
...
export const chatApi = {
  ...
  getChatList: async (): Promise<ChatRoom[]> => {
    try {
      const response = await chatAxiosInstance.get(API_ENDPOINTS.CHAT.LIST);
  ...
  getChatRoom: async (roomId: string): Promise<{ room: ChatRoom; messages: ChatMessage[] }> => {
    try {
      const response = await chatAxiosInstance.get(`${API_ENDPOINTS.CHAT.ROOM}/${roomId}`);
  ...
  sendMessage: async (data: SendMessageRequest): Promise<ChatMessage> => {
    try {
      const response = await chatAxiosInstance.post(API_ENDPOINTS.CHAT.SEND_MESSAGE, data);
